...
public class OrderPayActivity extends SuperActivity implements OnClickListener {
	...
	private int certiId;
	private RelativeLayout layout_service_card;
	private CardsBuyMyPet cardsBuyMyPet=null;
	...
	private void findView() {
		...
		layout_service_card = (RelativeLayout) findViewById(R.id.layout_service_card);
	...
	private void setView() {
		...
		if (previous == Global.BATH_TO_ORDERPAY) {
			...
			type = 1;
			payWays("care");
		} else if (previous==Global.BATHBEAY_NEW_TO_ORDERPAY) {//新版洗美
			uuid = fIntent.getStringExtra("uuid");
			orderNo = fIntent.getIntExtra("orderid", 0);
			couponid = fIntent.getIntExtra("couponid", 0);
			couponAmount = fIntent.getDoubleExtra("couponamount", 0);
			...
			cphone = fIntent.getStringExtra("cphone");
			remark = fIntent.getStringExtra("remark");
			selectedWorker = (Workers) fIntent.getSerializableExtra("selectedWorker");
			appointMentNewPetInfoList = (List<AppointMentNewPetInfo>) fIntent.getSerializableExtra("appointMentNewPetInfoList");
			addressid = fIntent.getIntExtra("addressid", 0);
			if (appointMentNewPetInfoList!=null&&appointMentNewPetInfoList.size()>0) {
				String subString = getServiceName(appointMentNewPetInfoList);
				if (subString.contains("1") && !subString.contains("2")) {
					servicename = "洗澡套餐金额";
				} else if (!subString.contains("1") && subString.contains("2")) {
					servicename = "美容套餐金额";
				...
				for (int i = 0; i < appointMentNewPetInfoList.size(); i++) {
					if (appointMentNewPetInfoList.get(i).getExtraItems()!=null) {
						List<ExtraItem>  listItems = appointMentNewPetInfoList.get(i).getExtraItems();
						for (int j = 0; j < listItems.size(); j++) {
							addservicefee+=listItems.get(j).getPrice();
						}
					}
				}
			}
			...
			layout_make_cards.setVisibility(View.VISIBLE);
			layout_make_cards_dog.setVisibility(View.GONE);
			layout_service_card.setVisibility(View.VISIBLE);
			cardsBuyMyPet = (CardsBuyMyPet) fIntent.getSerializableExtra("cardsBuyMyPet");
			textview_service_card.setText(cardsBuyMyPet.title);
			textview_service_card_price.setText("¥"+cardsBuyMyPet.price);
			textview_service_card_cut_down_detail.setText(cardsBuyMyPet.discount+"折");
			textview_service_card_cut_down_old_price_detail.setText("原价"+cardsBuyMyPet.listPrice);
			payfee = cardsBuyMyPet.price;
			textview_cards_totalmoney.setText("¥" + payfee);
			rl_orderpay_info.setVisibility(View.GONE);
			if (cardsBuyMyPet.contentList.size()>0) {
				StringBuilder builder = new StringBuilder();
				for (int i = 0; i < cardsBuyMyPet.contentList.size(); i++) {
					CardsBuyMyPet.Content content = cardsBuyMyPet.contentList.get(i);
					if (i==cardsBuyMyPet.contentList.size()-1) {
						builder.append(content.packageTip+" ¥"+content.pri+" * "+content.count+"次");
					}else {
						builder.append(content.packageTip+" ¥"+content.pri+" * "+content.count+"次" + "\n\n");
					}
				}
				if (!TextUtils.isEmpty(builder)) {
					textview_wash_title.setText(builder.toString());
				}
			}
			timerDown();
			payWays("timeCard");
	...
	public void onClick(View v) {
		...
		case R.id.bt_orderpay_pay:
			...
			if (paytype <= 0) {
				...
				if (previous == Global.BATH_TO_ORDERPAY) {
				...
				}else if (previous==Global.BATHBEAY_NEW_TO_ORDERPAY) {
					if (orderNo > 0) {
						if (balance <= 0 || (paytype == 4 && balance >= payfee)
								|| !isHybirdPay) {
							changepay(addservicefee, -1);
					...
					if (balance <= 0 || (paytype == 4 && balance >= payfee) || !isHybirdPay) {
						BuyCardNew(-1);
					} else {
						BuyCardNew(balance);
					}
	...
	private void goPayResult() {
		if (previous == Global.BATH_TO_ORDERPAY
			...
			if (CardsDetailNewActivity.cardsDetailNewActivity!=null) {
//				ToastUtil.showToastShortCenter(mContext, "购买此卡成功");
				CardsDetailNewActivity.cardsDetailNewActivity.finish();
//				setResult(Global.RESULT_OK);
	...
	private void goPaySuccess() {
		...
		if (previous == Global.UPDATE_TO_ORDERPAY) {
			...
			intent.putExtra("orderId", orderNo);
			intent.putExtra("petCustomerId", appointMentNewPetInfoList.get(0).getMyPetId());
		}else {
	...
	private void changepay(double foodfee, double balance) {
		...
		CommUtil.changeOrderPayMethodV2(yqm, spUtil.getString("cellphone", ""),
				...
				isVie, listIds, dataString,getPetID_ServiceId_MyPetId_ItemIds(appointMentNewPetInfoList),changeHanler);
	...
	private void changeUpdatePay(double balance) {
		...
		CommUtil.changeOrderPayMethodV2(null,
				...
				Utils.formatDouble(balance, 2), null, false, listIds,null,null,
				changeHanler);
	}
	...
	private void payByOrder(double balance) {
		...
		CommUtil.changeOrderPayMethodV2(yqm, spUtil.getString("cellphone", ""),
				...
				Utils.formatDouble(balance, 2), bathPetIds, isVie, listIds,dataString,getPetID_ServiceId_MyPetId_ItemIds(mpsList),
				changeHanler);
	}
	...
	private void sendOrder(double balance) {
		pDialog.showDialog();
		if (previous==Global.BATHBEAY_NEW_TO_ORDERPAY) {
			CommUtil.newOrder(this, spUtil.getString("cellphone", ""),
					Global.getIMEI(this), Global.getCurrentVersion(this),
					shopId, 0, couponid,selectedWorker.getWorkerId(),
					addressid, cname, cphone, addr, lat,
					lng,dataString, Utils.formatDouble(totalfee, 2),
					Utils.formatDouble(payfee, 2), paytype, remark,serviceloc,
					fIntent.getIntExtra("pickup", 0),0,
					getPetID_ServiceId_MyPetId_ItemIds(appointMentNewPetInfoList),
	...
	private void BuyCardNew(double balance){
		mPDialog.showDialog();
		if (cardsBuyMyPet!=null) {
			CommUtil.BuyCard(mContext, id, petid, -1, payfee,balance,paytype, cardsBuyMyPet.contentList.get(0).packageId,BuycardDetail);
		}
	}
	...
	public String getServiceName(List<AppointMentNewPetInfo> list) {
		if (list.size() <= 0) {
			return null;
		}
		StringBuffer sb = new StringBuffer();
