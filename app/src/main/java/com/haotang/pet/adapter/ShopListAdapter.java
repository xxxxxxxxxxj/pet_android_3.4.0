...
public class ShopListAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (swp != null) {
			GlideUtil.loadImg(act,swp.getImg(),
					...
					.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							if (!Utils.checkLogin(act)) {
								goLogin(LoginActivity.class,Global.SERVICE_NEW_TO_UNLOGINTOSHOP);
								return;
							}
							if (swp.getStatus() == 2) {
								...
								if (previous == Global.PRE_MAINFRAGMENT_TO_SERVICEACTIVITY_SHOPLIST
									...
									data.putExtra("lat", swp.getLat());
									data.putExtra("lng", swp.getLng());
	...
	private void goNext(Class clazz, int shopid, int areaid,int previous) {
	...
	private void goLogin(Class clazz,int previous){
		Intent intent = new Intent(act, clazz);
		intent.putExtra(Global.ANIM_DIRECTION(), Global.ANIM_COVER_FROM_RIGHT());
		((Activity) act).getIntent().putExtra(Global.ANIM_DIRECTION(),
				Global.ANIM_COVER_FROM_LEFT());
	...
	private void showPop(String imgUrl) {
		...
		if (pWin == null) {
			...
			GlideUtil.loadImg(act,imgUrl, imageView_not_practice, 0);
