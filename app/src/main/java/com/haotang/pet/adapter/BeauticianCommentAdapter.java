...
public class BeauticianCommentAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			holder.srivBeautician = (SelectableRoundedImageView) convertView.findViewById(R.id.sriv_beauticiancommentslist_item_beautician);
		...
		if (!TextUtils.isEmpty(list.get(position).memberIcon)) {
			...
			holder.imageview_tag.setTag(R.id.imageid, list.get(position).memberIcon);
			GlideUtil.loadImg(context,list.get(position).memberIcon,holder.imageview_tag,  0);
		}else {
			holder.imageview_tag.setVisibility(View.GONE);
		}
//		holder.srivBeautician.setTag(list.get(position).image);
//		holder.srivBeautician.setTag(R.id.imageid, list.get(position).image);
//		GlideUtil.loadImg(context,list.get(position).image,holder.srivBeautician,  R.drawable.user_icon_unnet);
		...
		if(list.get(position).images!=null){
			...
			for(int i=0; i<list.get(position).images.length;i++){
				if(i == 0){
					...
					holder.ivImage1.setTag(R.id.imageid, list.get(position).images[i]);
					GlideUtil.loadImg(context,list.get(position).images[i],holder.ivImage1,  R.drawable.icon_production_default);
				}else if(i == 1){
					holder.ivImage2.setVisibility(View.VISIBLE);
					holder.ivImage1.setTag(R.id.imageid, list.get(position).images[i]);
					GlideUtil.loadImg(context,list.get(position).images[i],holder.ivImage2,  R.drawable.icon_production_default);
				}else if(i == 2){
					holder.ivImage3.setVisibility(View.VISIBLE);
					holder.ivImage1.setTag(R.id.imageid, list.get(position).images[i]);
					GlideUtil.loadImg(context,list.get(position).images[i],holder.ivImage3,  R.drawable.icon_production_default);
				}
			}
		}
	...
	private class Holder{
		SelectableRoundedImageView srivBeautician;
		TextView tvName;
		TextView tvContent;
		TextView tvTime;
		ImageView ivImage1;
