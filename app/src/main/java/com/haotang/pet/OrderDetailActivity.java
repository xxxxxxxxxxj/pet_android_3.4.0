...
//				String strTime = timeLast[1].trim();
			...
			GlideUtil.loadImg(OrderDetailActivity.this,shopImg,srivPetStore, R.drawable.icon_production_default);
...
//			tvAddrType.setBackgroundResource(R.drawable.bg_red_round);
			GlideUtil.loadImg(OrderDetailActivity.this,ps.petstoreImg,srivPetStore,  0);
...
//						"#333333", "个)"));
				tvPickupPrice.setText(Utils.getTextAndColorFour(
						"#D1494F", "¥"+Utils.formatDouble(ps.pickupprice/**petSize*/), 
						"#D1494F", "(剩余:", 
						"#D1494F", ps.restAmount, 
						"#D1494F", "个)"));
			}
...
//			vPickupLine.setVisibility(View.VISIBLE);
			...
			tvPickupHot.setTextColor(Color.parseColor("#D1494F"));
...
//		addrid=1;
	...
	private void changepay(){
		...
		CommUtil.changeOrderPayMethodV2(null,spUtil.getString("cellphone", ""),
				...
				couponId,-1,null,isVie,getListIds(listIds),null,null,changeHanler);
	}
						...
						if (objectData.has("cards")&&!objectData.isNull("cards")) {
							...
							if (arrayCards.length()>0) {
								for (int i = 0; i < arrayCards.length(); i++) {
									...
									if (objectEveryCards.has("discountImg")&&!objectEveryCards.isNull("discountImg")) {
										confirmOrder.discountImg = objectEveryCards.getString("discountImg");
									}
