...
//							}
					...
					if (object.has("multiPet")&&!object.isNull("multiPet")) {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							if (objectMul.has("extraServiceItems")&&!objectMul.isNull("extraServiceItems")) {
								JSONArray arrayIds = objectMul.getJSONArray("extraServiceItems");
								if(arrayIds.length()>0){
									StringBuffer sb = new StringBuffer();
									for (int j = 0; j < arrayIds.length(); j++) {
										JSONObject objectIds = arrayIds.getJSONObject(j);
										sb.append(objectIds.getInt("ExtraServiceItemId")+",");
									}
									petServiceMul.addServiceIds = sb.substring(0, sb.toString().length()-1);
								}
							}
...
//			pWinCancle.setWidth(dm.widthPixels - 50);
	...
	private void sendOrder(){
		...
		CommUtil.changeOrderPayMethodV2(
				...
				paytype,couponId,-1,bathPetIds/*GetPetIds(mulPetServiceList)*/,isVie,getListIds(listIds),date,getPetID_ServiceId_MypetId_ItemIds(mulPetServiceList),orderHanler);
	}
