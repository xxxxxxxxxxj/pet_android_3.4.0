...
//			}
			...
			if (code==0) {
				BackUpdataPrice.clear();
				BackCardUpdataPrice.clear();
				allUpdataPrice.clear();
				allNoCardUpdataPrice.clear();
...
//						}
					...
					if (objectData.has("petItem")&&!objectData.isNull("petItem")) {
						...
						if (objectPetItem.has("tip")&&!objectPetItem.isNull("tip")) {
							tip = objectPetItem.getString("tip");
						}
						allUpdataPrice.add(updateEntity);
						allNoCardUpdataPrice.add(updateEntity);
					}
...
//			needMoney=0;
		CommUtil.changeOrderPayMethodV2(null,spUtil.getString("cellphone", ""),
				...
				/*pickup*/-1,-1,-1,Utils.formatDouble(needMoney,2),paytype,
				0,-1,null,false,getListIds(listIds),null,null,changeHanler);
	}
