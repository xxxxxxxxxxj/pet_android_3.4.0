...
public class WXPayEntryActivity extends Activity implements IWXAPIEventHandler {
	...
	public void onResp(BaseResp resp) {
		...
		if (resp.getType() == ConstantsAPI.COMMAND_PAY_BY_WX) {
			...
			Utils.mLogError("-----------RechargePage.rechargePage==null??"
					+ (RechargePage.rechargePage == null));
			if (resp.errCode == 0) {
				// 支付成功
				Utils.mLogError("----------errcode==0");
				try {
					if (RechargePage.rechargePage != null) {
						Log.e("TAG", "RechargePage.rechargePage != null");
						...
						Log.e("TAG", "CardsDetailActivity.detailActivity != null");
						CardsDetailActivity.detailActivity.finish();
						sendToOrderDetail();
					} else if (ADActivity.act != null) {
						Log.e("TAG", "ADActivity.aDActivity != null");
						Intent intent = new Intent();
						Bundle bundle = new Bundle();
						intent.setAction("android.intent.action.WXPayEntryActivity");
						bundle.putInt("previous",
								Global.ADACTIVITY_WXPAY_SUCCESS);
