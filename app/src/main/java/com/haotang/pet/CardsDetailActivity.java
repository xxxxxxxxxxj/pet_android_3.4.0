...
//								textView_cards_cannot_support.setText(notSupportCertiTip);
							...
							if (objectCard.has("cardImg")&&!objectCard.isNull("cardImg")) {
								...
								GlideUtil.loadImg(CardsDetailActivity.this,cardImg,imageview_show_back_img,  0);
							}
							cardsBuys.clear();
							cardMakeDogs.clear();
...
//											cardsBuy.content = objectEvery.getString("content");
	...
	private void goNext(CardsBuy cardsBuyEvery,Class cls,int pre) {//字段名起错了绕死了。
		Intent intent = new Intent(mContext,cls);
		intent.putExtra("CardsBuy", cardsBuyEvery);
		intent.putExtra("previous", pre);
		intent.putExtra("previous_liucheng", previous);//这个是老版的。。取名字绕着了。。
...
//			intent.putExtra("previous_liucheng", Global.SERVICE_NEW_TO_BUYCARD);
			intent.putExtra("id",id);
