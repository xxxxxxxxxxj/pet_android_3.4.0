...
public class PetInfoDeatilActivity extends SuperActivity implements OnClickListener {
	...
	private void setData() {
		if (pet != null) {
			...
			GlideUtil.loadImg(PetInfoDeatilActivity.this,pet.image, iv_petinfodetail_topbg,
		...
		case R.id.ll_petinfodetail_xz:// 洗澡
			...
			if (pet != null) {
				if (pet.availServiceType.contains("1")) {
					// 保存选择的宠物的头像
					goServiceType(ServiceNewActivity.class,1, pet.customerpetid);
		...
		case R.id.ll_petinfodetail_mr:// 美容
			...
			if (pet != null) {
				if (pet.availServiceType.contains("2")) {
					goServiceType(ServiceNewActivity.class,2, pet.customerpetid);
					// 保存选择的宠物的头像
					spUtil.saveString("petimg", pet.image);
				}else{
	...
	private void goServiceType(Class cls,int serviceType,int myPetId){
