	...
	public void onClick(View view) {
		...
		case R.id.bt_get:// 获取验证码
			if (CommUtil.getServiceBaseUrl().equals("http://192.168.0.252/")
				...
				if (et_userid_num.getText().toString().trim().equals("18510255744")||et_userid_num.getText().toString().trim().equals("15717155675")) {
			...
			try {
				...
				if (0 == resultCode && jobj.has("data") && !jobj.isNull("data")) {
					...
					if (previous == Global.PRE_MALLFRAGMENT_TO_LOGIN_BACK_MALLFRAGMENT) {
						...
						startActivity(intent);
						finishWithAnimation();
					}else if (previous==Global.SERVICE_NEW_TO_UNLOGINTOSHOP) {
