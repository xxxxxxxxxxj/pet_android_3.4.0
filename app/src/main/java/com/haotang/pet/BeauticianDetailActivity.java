...
public class BeauticianDetailActivity extends SuperActivity {
	...
	private void findView() {
		...
		// layout_speciality_one = (LinearLayout)
		// findViewById(R.id.layout_speciality_one);
		// layout_speciality_two = (LinearLayout)
		// findViewById(R.id.layout_speciality_two);
		// textview_speciality_one = (TextView)
		// findViewById(R.id.textview_speciality_one);
		// textview_speciality_two = (TextView)
		// findViewById(R.id.textview_speciality_two);
	...
	private void setView() {
		...
		btBooking.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				if (previous == Global.APPOINTNEW_TO_BEAUDETAI) {
					finishWithAnimation();
				} else if (previous == Global.MAIN_TO_BEAUTICIANLIST) {
	...
	private void getData(int id, int FromAreaid) {
		pDialog.showDialog();
		CommUtil.getBeauticianDetail(this, id, FromAreaid, apptime,
				BeauticianHanler);
	}
		...
		public void onSuccess(int statusCode, Header[] headers,
			...
			try {
				...
				if (0 == resultCode && jobj.has("data") && !jobj.isNull("data")) {
					...
					// if (objectExp.has("itemName")&&
					// !objectExp.isNull("itemName")) {
					...
					if (jData.has("introduction")
						...
						SpannableString spans = new SpannableString(
								introduction);
						// spans.setSpan(new
						// ForegroundColorSpan(getResources().getColor(R.color.orange)),
					...
					// tv_servicedetail_goodcomments.setText(Utils.getTextAndColorComments("好评：",csJson.getInt("hp")
					// + "条"));
					// }
					...
					if (jData.has("goodRate") && !jData.isNull("goodRate")) {
						tv_servicedetail_goodcomments.setText("好评率："
								+ (int) (jData.getDouble("goodRate") * 100)
								+ "%");
					}
					if (jData.has("grade") && !jData.isNull("grade")) {
						tv_servicedetail_badcomments.setText("评分："
								+ jData.getDouble("grade"));
					}
	...
	private void setBeauticianProduction(int index, Production pr) {
		...
		case 1:
			if (pr.smallimage != null && !"".equals(pr.smallimage.trim()))
				GlideUtil.loadImg(BeauticianDetailActivity.this,pr.smallimage, srivProduction1,
						R.drawable.icon_image_default);

			break;
		case 2:
			if (pr.smallimage != null && !"".equals(pr.smallimage.trim()))
				GlideUtil.loadImg(BeauticianDetailActivity.this,pr.smallimage, srivProduction2,
						R.drawable.icon_image_default);

			break;
		case 3:
			if (pr.smallimage != null && !"".equals(pr.smallimage.trim()))
				GlideUtil.loadImg(BeauticianDetailActivity.this,pr.smallimage, srivProduction3,
						R.drawable.icon_image_default);

			break;
		case 4:
			if (pr.smallimage != null && !"".equals(pr.smallimage.trim()))
				GlideUtil.loadImg(BeauticianDetailActivity.this,pr.smallimage, srivProduction4,
						R.drawable.icon_image_default);

			break;
		case 5:
			if (pr.smallimage != null && !"".equals(pr.smallimage.trim()))
				GlideUtil.loadImg(BeauticianDetailActivity.this,pr.smallimage, srivProduction5,
						R.drawable.icon_image_default);

			break;
		case 6:
			if (pr.smallimage != null && !"".equals(pr.smallimage.trim()))
				GlideUtil.loadImg(BeauticianDetailActivity.this,pr.smallimage, srivProduction6,
						R.drawable.icon_image_default);
		...
		if (array.length() == 0) {
			...
			if (commentOne != null) {
				...
				if (!commentOne.image.equals("")) {
					GlideUtil.loadImg(BeauticianDetailActivity.this,commentOne.image,beau_icon,  0);
				}
				if (!TextUtils.isEmpty(commentOne.memberIcon)
						&& commentOne.memberIcon != null) {
					service_eva_is_member_one.setVisibility(View.VISIBLE);
					if (!commentOne.memberIcon.startsWith("http")) {
						commentOne.memberIcon = CommUtil.getServiceNobacklash()
								+ commentOne.memberIcon;
					}
					GlideUtil.loadImg(BeauticianDetailActivity.this,commentOne.memberIcon,service_eva_is_member_one,
							 0);
				} else {
			...
			if (commentTwo != null) {
				...
				if (!commentTwo.image.equals("")) {
					GlideUtil.loadImg(BeauticianDetailActivity.this,commentTwo.image, beau_icon_two, 0);
				}
				if (!TextUtils.isEmpty(commentTwo.memberIcon)
						&& commentTwo.memberIcon != null) {
					service_eva_is_member_two.setVisibility(View.VISIBLE);
					if (!commentTwo.memberIcon.startsWith("http")) {
						commentTwo.memberIcon = CommUtil.getServiceNobacklash()
								+ commentTwo.memberIcon;
					}
					GlideUtil.loadImg(BeauticianDetailActivity.this,commentTwo.memberIcon,service_eva_is_member_two,
							 0);
				} else {
	...
	private void setBeauEvaShow(Comment comment, ImageView ShowOne,
		...
		if (comment.images.length == 1) {
			...
			GlideUtil.loadImg(BeauticianDetailActivity.this,comment.images[0], ShowOne,
					R.drawable.icon_beautician_default);
		} else if (comment.images.length == 2) {
			ShowOne.setVisibility(View.VISIBLE);
			ShowTwo.setVisibility(View.VISIBLE);
			GlideUtil.loadImg(BeauticianDetailActivity.this,comment.images[0], ShowOne,
					R.drawable.icon_beautician_default);
			GlideUtil.loadImg(BeauticianDetailActivity.this,comment.images[1], ShowTwo,
					R.drawable.icon_beautician_default);
			...
			GlideUtil.loadImg(BeauticianDetailActivity.this,comment.images[1], ShowTwo,
					R.drawable.icon_beautician_default);
			GlideUtil.loadImg(BeauticianDetailActivity.this,comment.images[2], ShowThr,
					R.drawable.icon_beautician_default);
		}
	}
	...
	private void setHintExt() {
		...
		if (isShrink) {
			...
			isShrink = true;
			relayout_inside_img
					.setImageResource(R.drawable.icon_arrow_down_beau);
