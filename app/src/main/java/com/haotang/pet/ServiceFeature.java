...
public class ServiceFeature extends SuperActivity implements OnClickListener {
		...
		first_item_service_one.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(Utils.checkLogin(mContext)){
					...
					startActivity(new Intent(ServiceFeature.this,LoginActivity.class));
				}
			}
		});
	...
	private void setView() {
		...
		if (previous == Global.SERVICEFEATURE_TO_PETLIST
				...
				|| previous == Global.SERVICEFEATURE_TO_PETLIST_SHOPLIST) {
	...
	private void getData(int petid) {
		...
		if (fIntent.getIntExtra("beautician_id", 0)<=0) {
			...
			CommUtil.getFeaturedService1(spUtil.getString("cellphone", ""),
					Global.getIMEI(this), this, petid, typeId, beautician_sort,
					fIntent.getIntExtra("beautician_id", 0), shopid,
					getFeaturedService1);
	}
			...
			try {
				...
				if (code == 0) {
					if (object.has("data") && !object.isNull("data")) {
						...
						if (objectData.has("activityCopy")
								&& !objectData.isNull("activityCopy")) {
							// String activityCopy =
							// objectData.getString("activityCopy");
							// textview_give_other_money.setText(activityCopy);
							JSONObject objectActivityCopy = objectData
									.getJSONObject("activityCopy");
							if (objectActivityCopy.has("txt")
									&& !objectActivityCopy.isNull("txt")) {
								textview_give_other_money
										.setText(objectActivityCopy
												.getString("txt"));
							}
							if (objectActivityCopy.has("url")
									&& !objectActivityCopy.isNull("url")) {
								BuyCardH5Url = objectActivityCopy
										.getString("url");
							}
						} else {
							textview_give_other_money.setVisibility(View.GONE);
						}
						if (objectData.has("nearbyDogs")
								&& !objectData.isNull("nearbyDogs")) {
							textView_service_detail_num_to_be_use
									.setText(objectData.getInt("nearbyDogs")
											+ "");
						}
						if (objectData.has("petServiceType")
								&& !objectData.isNull("petServiceType")) {
							JSONObject objectType = objectData
									.getJSONObject("petServiceType");
							if (objectType.has("name")
									&& !objectType.isNull("name")) {
								serviceCurrentName = objectType
										.getString("name");
							}
						}
						if (objectData.has("evaluateObj")
								&& !objectData.isNull("evaluateObj")) {
							JSONObject objectEva = objectData
						...
						if (objectData.has("reminders")
								&& !objectData.isNull("reminders")) {
							JSONArray arrayReminders = objectData
						...
						if (objectData.has("allServices")
								&& !objectData.isNull("allServices")) {
							JSONArray array = objectData
									.getJSONArray("allServices");
							if (array.length() != 0) {
						...
						if (objectData.has("pet") && !objectData.isNull("pet")) {
							JSONObject objectPet = objectData
									.getJSONObject("pet");
							Pet pet = Pet.json2Entity(objectPet);
							petid = pet.id;
							if (!"".equals(pet.name)) {
								// if(serviceCurrentName != null &&
								// serviceCurrentName.length() >=2){
								// if(serviceCurrentName.substring(serviceCurrentName.length()-2,
								// serviceCurrentName.length()).contains("套餐")){
								// serviceCurrentName =
								// serviceCurrentName.substring(0,
								// serviceCurrentName.length()-2);
								// }
								// }
								if (customerpetid > 0) {
									textView_service_name_show
											.setText(customerpetname + "专属"
													+ serviceCurrentName);
						...
						if (objectData.has("services")
								&& !objectData.isNull("services")) {
							// sbServiceIds = new StringBuffer();
							featureList.clear();
							JSONArray array = objectData
									.getJSONArray("services");
							for (int i = 0; i < array.length(); i++) {
								featureList.add(ServicesFeature
						...
						if (objectData.has("picIntro")
								&& !objectData.isNull("picIntro")) {
							Imagelist.clear();
							JSONArray arrayPic = objectData
									.getJSONArray("picIntro");
						...
						if (objectData.has("comments")
								&& !objectData.isNull("comments")) {
						...
						if (objectData.has("positiveComments")
								&& !objectData.isNull("positiveComments")) {
							if (objectData.getInt("positiveComments") > 999) {
								tv_servicedetail_goodcomments
								...
								tv_servicedetail_goodcomments
										.setText(Utils.getTextAndColorComments(
												"好评：",
												objectData
														.getInt("positiveComments")
														+ "条"));
							}
						}
						if (objectData.has("negativeComments")
								&& !objectData.isNull("negativeComments")) {
							tv_servicedetail_badcomments.setText(Utils
									.getTextAndColorComments(
											"差评：",
											objectData
													.getInt("negativeComments")
													+ "条"));
						}
						if (objectData.has("levelDesc")
								&& !objectData.isNull("levelDesc")) {
							JSONObject objectLevelDesc = objectData
							...
							if (objectLevelDesc.has("10")
								...
								if (object10.has("title")
									...
									first_one_service_name.setText(object10
											.getString("title"));
									Second_one_service_name.setText(object10
											.getString("title"));
									Thr_one_service_name.setText(object10
											.getString("title"));
								}
								if (object10.has("desc")
										&& !object10.isNull("desc")) {
									first_one_service_detail.setText(object10
											.getString("desc"));
									Second_one_service_detail.setText(object10
											.getString("desc"));
									Thr_one_service_detail.setText(object10
											.getString("desc"));
								}
							}
							if (objectLevelDesc.has("20")
								...
								if (object20.has("title")
									...
									first_two_service_name.setText(object20
											.getString("title"));
									Second_two_service_name.setText(object20
											.getString("title"));
									Thr_two_service_name.setText(object20
											.getString("title"));
								}
								if (object20.has("desc")
										&& !object20.isNull("desc")) {
									first_two_service_detail.setText(object20
											.getString("desc"));
									Second_two_service_detail.setText(object20
											.getString("desc"));
									Thr_two_service_detail.setText(object20
											.getString("desc"));
							...
							if (objectLevelDesc.has("30")
								...
								if (object30.has("title")
									...
									first_thr_service_name.setText(object30
											.getString("title"));
									Second_thr_service_name.setText(object30
											.getString("title"));
									Thr_thr_service_name.setText(object30
											.getString("title"));
								}
								if (object30.has("desc")
										&& !object30.isNull("desc")) {
									first_thr_service_detail.setText(object30
											.getString("desc"));
									Second_thr_service_detail.setText(object30
											.getString("desc"));
									Thr_thr_service_detail.setText(object30
											.getString("desc"));
								}
							}
						}
						if (objectData.has("banners")
								&& !objectData.isNull("banners")
								&& objectData.getJSONArray("banners").length() > 0) {
							listBanner.clear();
							JSONArray bannersArray = objectData
	...
	private void setThrShow() {
		...
		try {
			ImageLoaderUtil.loadImg(featureThr.spic,
					imageview_scaling_and_beau_icon, defaultImgId,null);
		} catch (Exception e1) {
		...
		try {
			...
			textview_scaling_and_beau_price.setText(styledText,
					TextView.BufferType.SPANNABLE);
			textview_price_thr.setText(featureThr.minPrice + "");
			textview_fea_thr_price.setText(getTextNewPrice("¥"
					+ featureThr.minPrice));
			no_old_price_thr_item.setText(getTextNewPrice("¥"
					+ featureThr.minPrice));
			...
			if (beautician_sort == 10) {
				old_thr_list_price
						.setText(featureThr.ServicePrice.shopListPrice);
				Utils.mLogError("==-->featureThr.ServicePrice.shopListPrice "
						+ featureThr.ServicePrice.shopListPrice);
				...
				old_thr_list_price
						.setText(featureThr.ServicePrice.shopListPrice20);
				...
				old_thr_list_price
						.setText(featureThr.ServicePrice.shopListPrice30);
		...
		if (featureThr.ServicePrice.shopPrice10 != -1) {
			Thr_one_service_price.setText(getTextNewPrice("¥"
					+ featureThr.ServicePrice.shopPrice10));
			Thr_no_old_price_one.setText(getTextNewPrice("¥"
					+ featureThr.ServicePrice.shopPrice10));
			if (TextUtils.isEmpty(featureThr.ServicePrice.shopListPrice)) {
		...
		if (featureThr.ServicePrice.shopPrice20 != -1) {
			Thr_two_service_price.setText(getTextNewPrice("¥"
					+ featureThr.ServicePrice.shopPrice20));

			Thr_no_old_price_two.setText(getTextNewPrice("¥"
					+ featureThr.ServicePrice.shopPrice20));
			if (TextUtils.isEmpty(featureThr.ServicePrice.shopListPrice20)) {
		...
		if (featureThr.ServicePrice.shopPrice30 != -1) {
			Thr_thr_service_price.setText(getTextNewPrice("¥"
					+ featureThr.ServicePrice.shopPrice30));
			Thr_no_old_price_thr.setText(getTextNewPrice("¥"
					+ featureThr.ServicePrice.shopPrice30));
			if (TextUtils.isEmpty(featureThr.ServicePrice.shopListPrice30)) {
		...
		Thr_item_service_one.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
		Thr_item_service_two.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
		Thr_item_service_thr.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
	}
	...
	private void setTwoShow() {
		...
		if (featureTwo.name != null && featureTwo.name.contains("洗澡"))
			...
			defaultImgId = R.drawable.icon_special_spabeauty;
		try {
			ImageLoaderUtil.loadImg(featureTwo.spic,
					imageview_scaling_and_wash_icon, defaultImgId,null);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		try {
			...
			textview_scaling_and_wash_price.setText(styledText,
					TextView.BufferType.SPANNABLE);
			textview_price_two.setText(featureTwo.minPrice + "");
			textview_fea_two_price.setText(getTextNewPrice("¥"
					+ featureTwo.minPrice));
			no_old_price_two_item.setText(getTextNewPrice("¥"
					+ featureTwo.minPrice));
			Second_one_old_price.setText(featureTwo.ServicePrice.shopListPrice);
			Second_two_old_price
					.setText(featureTwo.ServicePrice.shopListPrice20);
			Second_thr_old_price
					.setText(featureTwo.ServicePrice.shopListPrice30);

			if (beautician_sort == 10) {
				old_two_list_price
						.setText(featureTwo.ServicePrice.shopListPrice);
				Utils.mLogError("==-->featureTwo.ServicePrice.shopListPrice "
						+ featureTwo.ServicePrice.shopListPrice);
		...
		if (featureTwo.ServicePrice.shopPrice10 != -1) {
			Second_one_service_price.setText(getTextNewPrice("¥"
					+ featureTwo.ServicePrice.shopPrice10));
			Second_no_old_price_one.setText(getTextNewPrice("¥"
					+ featureTwo.ServicePrice.shopPrice10));
			if (TextUtils.isEmpty(featureTwo.ServicePrice.shopListPrice)) {
		...
		if (featureTwo.ServicePrice.shopPrice20 != -1) {
			Second_two_service_price.setText(getTextNewPrice("¥"
					+ featureTwo.ServicePrice.shopPrice20));
			Second_no_old_price_two.setText(getTextNewPrice("¥"
					+ featureTwo.ServicePrice.shopPrice20));
			if (TextUtils.isEmpty(featureTwo.ServicePrice.shopListPrice20)) {
		...
		if (featureTwo.ServicePrice.shopPrice30 != -1) {
			Second_thr_service_price.setText(getTextNewPrice("¥"
					+ featureTwo.ServicePrice.shopPrice30));
			Second_no_old_price_thr.setText(getTextNewPrice("¥"
					+ featureTwo.ServicePrice.shopPrice30));
			if (TextUtils.isEmpty(featureTwo.ServicePrice.shopListPrice30)) {
		...
		Second_item_service_one.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
		Second_item_service_two.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
		Second_item_service_thr.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
	...
	private void setOneShow() {
		...
		try {
			...
			textview_scaling_price.setText(styledText,
					TextView.BufferType.SPANNABLE);
			textview_price_one.setText(featureOne.minPrice + "");
			textview_fea_one_price.setText(getTextNewPrice("¥"
					+ featureOne.minPrice));
			no_old_price_one_item.setText(getTextNewPrice("¥"
					+ featureOne.minPrice));

			first_one_old_price.setText(featureOne.ServicePrice.shopListPrice);
			first_two_old_price
					.setText(featureOne.ServicePrice.shopListPrice20);
			first_thr_old_price
					.setText(featureOne.ServicePrice.shopListPrice30);
			if (beautician_sort == 10) {
				old_one_list_price
						.setText(featureOne.ServicePrice.shopListPrice);
				Utils.mLogError("==-->featureOne.ServicePrice.shopListPrice "
						+ featureOne.ServicePrice.shopListPrice);
		...
		if (featureOne.name != null && featureOne.name.contains("洗澡"))
			...
			defaultImgId = R.drawable.icon_special_spabeauty;
		try {
			ImageLoaderUtil.loadImg(featureOne.spic, imageview_scaling_icon,
					defaultImgId,null);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		if (featureOne.ServicePrice.shopPrice10 != -1) {
			first_one_service_price.setText(getTextNewPrice("¥"
					+ featureOne.ServicePrice.shopPrice10));

			first_no_old_price_one.setText(getTextNewPrice("¥"
					+ featureOne.ServicePrice.shopPrice10));
			if (TextUtils.isEmpty(featureOne.ServicePrice.shopListPrice)) {
		...
		if (featureOne.ServicePrice.shopPrice20 != -1) {
			first_two_service_price.setText(getTextNewPrice("¥"
					+ featureOne.ServicePrice.shopPrice20));
			first_no_old_price_two.setText(getTextNewPrice("¥"
					+ featureOne.ServicePrice.shopPrice20));
		...
		if (featureOne.ServicePrice.shopPrice30 != -1) {
			first_thr_service_price.setText(getTextNewPrice("¥"
					+ featureOne.ServicePrice.shopPrice30));
			first_no_old_price_thr.setText(getTextNewPrice("¥"
					+ featureOne.ServicePrice.shopPrice30));
		...
		first_item_service_one.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
		first_item_service_two.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
		first_item_service_thr.setBackgroundColor(getResources().getColor(
				R.color.f8f8ff8));
	...
	private SpannableString getTextPrice(String lastOnePrice) {
		...
		styledText.setSpan(new TextAppearanceSpan(this, R.style.style0), 0, 1,
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		styledText.setSpan(new TextAppearanceSpan(this, R.style.style1), 1,
	...
	private void findView() {
		...
		footer = LayoutInflater.from(this).inflate(
				R.layout.item_eva_service_feature, null);
		service_layout_eva_one = (LinearLayout) footer
				.findViewById(R.id.service_layout_eva_one);
		service_layout_eva_two = (LinearLayout) footer
				.findViewById(R.id.service_layout_eva_two);
		service_eva_is_member_one = (ImageView) footer
				.findViewById(R.id.service_eva_is_member_one);
		service_eva_is_member_two = (ImageView) footer
				.findViewById(R.id.service_eva_is_member_two);
		item_eva_layout = (LinearLayout) footer
				.findViewById(R.id.item_eva_layout);
		...
		textView_service_eva_one = (TextView) footer
				.findViewById(R.id.textView_service_eva_one);
		textView_service_eva_two = (TextView) footer
				.findViewById(R.id.textView_service_eva_two);

		tv_servicedetail_goodcomments = (TextView) footer
				.findViewById(R.id.tv_servicedetail_goodcomments);
		tv_servicedetail_badcomments = (TextView) footer
				.findViewById(R.id.tv_servicedetail_badcomments);
		...
		// button_go_apponit_scaling = (Button)
		// findViewById(R.id.button_go_apponit_scaling);
		...
		// button_go_apponit_scaling_and_wash = (Button)
		// findViewById(R.id.button_go_apponit_scaling_and_wash);
		...
		// button_go_apponit_scaling_and_beau = (Button)
		// findViewById(R.id.button_go_apponit_scaling_and_beau);
		...
		// item one
		first_item_service_one = (LinearLayout) one_service
				.findViewById(R.id.item_service_one);
		first_one_service_name = (TextView) one_service
				.findViewById(R.id.textview_item_service_name_one);
		first_one_service_detail = (TextView) one_service
				.findViewById(R.id.textview_item_service_detail_one);
		first_one_service_price = (TextView) one_service
				.findViewById(R.id.textview_item_service_price_one);
		first_one_old_price = (TextView) one_service
				.findViewById(R.id.old_one_price);
		first_no_old_price_one = (TextView) one_service
				.findViewById(R.id.no_old_price_one);
		first_one_button_apponit = (Button) one_service
				.findViewById(R.id.button_go_apponit_one);
		// item two
		first_item_service_two = (LinearLayout) one_service
				.findViewById(R.id.item_service_two);
		first_two_service_name = (TextView) one_service
				.findViewById(R.id.textview_item_service_name_two);
		first_two_service_detail = (TextView) one_service
				.findViewById(R.id.textview_item_service_detail_two);
		first_two_service_price = (TextView) one_service
				.findViewById(R.id.textview_item_service_price_two);
		first_two_old_price = (TextView) one_service
				.findViewById(R.id.old_two_price);
		first_no_old_price_two = (TextView) one_service
				.findViewById(R.id.no_old_price_two);
		first_two_button_apponit = (Button) one_service
				.findViewById(R.id.button_go_apponit_two);
		// item thr
		first_item_service_thr = (LinearLayout) one_service
				.findViewById(R.id.item_service_thr);
		first_thr_service_name = (TextView) one_service
				.findViewById(R.id.textview_item_service_name_thr);
		first_thr_service_detail = (TextView) one_service
				.findViewById(R.id.textview_item_service_detail_thr);
		first_thr_service_price = (TextView) one_service
				.findViewById(R.id.textview_item_service_price_thr);
		first_thr_old_price = (TextView) one_service
				.findViewById(R.id.old_thr_price);
		first_no_old_price_thr = (TextView) one_service
				.findViewById(R.id.no_old_price_thr);
		first_thr_button_apponit = (Button) one_service
				.findViewById(R.id.button_go_apponit_thr);
		...
		// item one
		Second_item_service_one = (LinearLayout) two_service
				.findViewById(R.id.item_service_one);
		Second_one_service_name = (TextView) two_service
				.findViewById(R.id.textview_item_service_name_one);
		Second_one_service_detail = (TextView) two_service
				.findViewById(R.id.textview_item_service_detail_one);
		Second_one_service_price = (TextView) two_service
				.findViewById(R.id.textview_item_service_price_one);
		Second_one_old_price = (TextView) two_service
				.findViewById(R.id.old_one_price);
		Second_no_old_price_one = (TextView) two_service
				.findViewById(R.id.no_old_price_one);
		Second_one_button_apponit = (Button) two_service
				.findViewById(R.id.button_go_apponit_one);
		// item two
		Second_item_service_two = (LinearLayout) two_service
				.findViewById(R.id.item_service_two);
		Second_two_service_name = (TextView) two_service
				.findViewById(R.id.textview_item_service_name_two);
		Second_two_service_detail = (TextView) two_service
				.findViewById(R.id.textview_item_service_detail_two);
		Second_two_service_price = (TextView) two_service
				.findViewById(R.id.textview_item_service_price_two);
		Second_two_old_price = (TextView) two_service
				.findViewById(R.id.old_two_price);
		Second_no_old_price_two = (TextView) two_service
				.findViewById(R.id.no_old_price_two);
		Second_two_button_apponit = (Button) two_service
				.findViewById(R.id.button_go_apponit_two);
		// item thr
		Second_item_service_thr = (LinearLayout) two_service
				.findViewById(R.id.item_service_thr);
		Second_thr_service_name = (TextView) two_service
				.findViewById(R.id.textview_item_service_name_thr);
		Second_thr_service_detail = (TextView) two_service
				.findViewById(R.id.textview_item_service_detail_thr);
		Second_thr_service_price = (TextView) two_service
				.findViewById(R.id.textview_item_service_price_thr);
		Second_thr_old_price = (TextView) two_service
				.findViewById(R.id.old_thr_price);
		Second_no_old_price_thr = (TextView) two_service
				.findViewById(R.id.no_old_price_thr);
		Second_thr_button_apponit = (Button) two_service
				.findViewById(R.id.button_go_apponit_thr);
		...
		// item one
		Thr_item_service_one = (LinearLayout) thr_service
				.findViewById(R.id.item_service_one);
		Thr_one_service_name = (TextView) thr_service
				.findViewById(R.id.textview_item_service_name_one);
		Thr_one_service_detail = (TextView) thr_service
				.findViewById(R.id.textview_item_service_detail_one);
		Thr_one_service_price = (TextView) thr_service
				.findViewById(R.id.textview_item_service_price_one);
		Thr_one_old_price = (TextView) thr_service
				.findViewById(R.id.old_one_price);
		Thr_no_old_price_one = (TextView) thr_service
				.findViewById(R.id.no_old_price_one);
		Thr_one_button_apponit = (Button) thr_service
				.findViewById(R.id.button_go_apponit_one);
		// item two
		Thr_item_service_two = (LinearLayout) thr_service
				.findViewById(R.id.item_service_two);
		Thr_two_service_name = (TextView) thr_service
				.findViewById(R.id.textview_item_service_name_two);
		Thr_two_service_detail = (TextView) thr_service
				.findViewById(R.id.textview_item_service_detail_two);
		Thr_two_service_price = (TextView) thr_service
				.findViewById(R.id.textview_item_service_price_two);
		Thr_two_old_price = (TextView) thr_service
				.findViewById(R.id.old_two_price);
		Thr_no_old_price_two = (TextView) thr_service
				.findViewById(R.id.no_old_price_two);
		Thr_two_button_apponit = (Button) thr_service
				.findViewById(R.id.button_go_apponit_two);
		// item thr
		Thr_item_service_thr = (LinearLayout) thr_service
				.findViewById(R.id.item_service_thr);
		Thr_thr_service_name = (TextView) thr_service
				.findViewById(R.id.textview_item_service_name_thr);
		Thr_thr_service_detail = (TextView) thr_service
				.findViewById(R.id.textview_item_service_detail_thr);
		Thr_thr_service_price = (TextView) thr_service
				.findViewById(R.id.textview_item_service_price_thr);
		Thr_thr_old_price = (TextView) thr_service
				.findViewById(R.id.old_thr_price);
		Thr_no_old_price_thr = (TextView) thr_service
				.findViewById(R.id.no_old_price_thr);
		Thr_thr_button_apponit = (Button) thr_service
				.findViewById(R.id.button_go_apponit_thr);
	...
	public void onClick(View v) {
		...
		case R.id.ll_servicedetail_changepet:
			...
			goNextForData(ChoosePetActivityNew.class,
					Global.SERVICEFEATURE_TO_PETLIST_1);
			break;
	...
	private void goAppoinement(int index) {
		...
		intent.putExtra("beautician_sortname",
		...
		try {
			...
			if (array.length() == 0) {
				...
				if (objectOne.has("avatar") && !objectOne.isNull("avatar")) {
					GlideUtil.loadImg(ServiceFeature.this,CommUtil.getServiceNobacklash()
							+ objectOne.getString("avatar"), service_eva_one,
							0);
				}
					...
					if (objectUser.has("userMemberLevel")
						...
						if (objectUserLevel.has("memberIcon")
							...
							if (!memberIcon.startsWith("http")) {
								memberIcon = CommUtil.getServiceNobacklash()
										+ memberIcon;
							}
							GlideUtil.loadImg(ServiceFeature.this,memberIcon,service_eva_is_member_one,
									 0);
						} else {
				...
				if (objectTwo.has("avatar") && !objectTwo.isNull("avatar")) {
					GlideUtil.loadImg(ServiceFeature.this,CommUtil.getServiceNobacklash()
							+ objectTwo.getString("avatar"), service_eva_two,
							0);
				}
				if (objectOne.has("user") && !objectOne.isNull("user")) {
					JSONObject objectUser = objectOne.getJSONObject("user");
				...
				if (objectTwo.has("user") && !objectTwo.isNull("user")) {
					...
					if (objectUser.has("userMemberLevel")
						...
						if (objectUserLevel.has("memberIcon")
							...
							GlideUtil.loadImg(ServiceFeature.this,memberIcon,service_eva_is_member_two,
									 0);
						} else {
	...
	private void addQQQZonePlatform() {
		...
		// 添加QZone平台
		QZoneSsoHandler qZoneSsoHandler = new QZoneSsoHandler(act, appId,
				appKey);
